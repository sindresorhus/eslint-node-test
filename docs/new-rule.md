...
- Look at how previous rules were added as inspiration (browse the rule files in `rules/` and their tests in `test/`).
- Read [`AGENTS.md`](../AGENTS.md), especially the “Detecting `node:test`” section and the shared helpers in `rules/utils/node-test.js`.
...
- Run `npm run fix:eslint-docs` so the rule is added to the readme table and the doc header is generated.
- Run `npm test` to ensure the tests pass.
- Open a pull request with a title in exactly the format `` Add `rule-name` rule ``, for example, `` Add `no-only-test` rule ``.
