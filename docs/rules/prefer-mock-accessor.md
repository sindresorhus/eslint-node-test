...
This rule reports `mock.method()` and `t.mock.method()` calls whose effective options are a statically analyzable, unspread object literal that enables `getter` or `setter` with a literal `true` value. For four-argument calls, the implementation must be inline so the autofix preserves Node.js's overload resolution. It ignores call-argument spreads and computed option keys because they can make the effective argument positions or options dynamic. It also ignores object-literal getters and `__proto__` properties because the dedicated APIs rebuild the options object. The autofix changes only `.method`, preserving the options object exactly. You can remove the now-redundant accessor option separately if desired.
