...
This rule reports `mock.method()` and `t.mock.method()` calls whose effective options are an unspread object literal that enables `getter` or `setter` with a literal `true` value. It ignores object-literal getters and `__proto__` properties because the dedicated APIs rebuild the options object. The autofix changes only `.method`, preserving the options object exactly. You can remove the now-redundant accessor option separately if desired.
