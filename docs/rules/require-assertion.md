...
A test without any assertions will always pass, even if the code under test is broken. This rule requires each `test`/`it` call to contain at least one assertion from `node:assert` or `t.assert`.
