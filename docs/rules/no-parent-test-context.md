...
This rule reports references to an outer test context binding from inside an inline subtest callback. Use the child callback's own context parameter instead. Non-inline subtest callbacks such as `t.test('child', helper)` are not analyzed.
